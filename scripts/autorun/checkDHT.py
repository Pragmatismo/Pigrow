...
def humid_contol(humid, use_fans=False):
    ...
    if humid < humid_low and humid_state != 'up_on':
        msg = "should turn the humidifer on, it's " + \
            str(humid) + " and the low limit is " + str(humid_low)
        if humid_state == 'unknown':
            msg = "Script initialised, humid " + \
                str(humid) + ", low limit is " + \
                str(humid_low) + " checking humidifier is on"
        humid_state = 'up_on'
        ...
        msg = (
            "should turn the humidifier off, it's " +
            str(humid) +
            " and the low limit is " +
            str(humid_low))
        if humid_state == 'unknown':
            msg = "Script initialised, humid " + \
                str(humid) + ", low limit is " + \
                str(humid_low) + " checking humidifier is off"
        humid_state = 'up_off'
        pigrow_defs.write_log(script, msg, loc_dic['loc_switchlog'])
...
def dehumid_control(humid, use_fans=False):
    ...
    if humid > humid_high and dehumid_state != 'down_on':
        msg = "should turn dehumidifer on, it's " + \
            str(humid) + " and the high limit is " + str(humid_high)
        if dehumid_state == 'unknown':
            msg = "Script initialised, humid " + \
                str(humid) + ", the high limit is " + \
                str(humid_high) + " checking it's on"
        ...
        msg = "should turn dehumid off, it's " + \
            str(humid) + " and the high limit is " + str(humid_high)
        if dehumid_state == 'unknown':
            msg = "Script initialised, humid " + \
                str(humid) + ", high limit is " + \
                str(humid_high) + " checking it's off"
...
if change:
    ...
    print("Not matching, problem with time thingy! ir was " +
          str(state) + " having a rest then trying again...")
...
while True:
    try:
        ...
        print(
            ...
            str(humid))
        if not timno == '-1':
            if not str(set_dic['gpio_heater']).strip(
            ) == '' or log_non == True or heat_use_fan == True:
                if use_heat == True:
                    heater_control(temp, heat_use_fan)
            if not str(set_dic['gpio_humid']).strip(
            ) == '' or log_non == True or hum_use_fan == True:
                if use_humid == True:
                    humid_contol(humid, hum_use_fan)
            if not str(set_dic['gpio_dehumid']).strip(
            ) == '' or log_non == True or dehum_use_fan == True:
                if use_dehumid == True:
