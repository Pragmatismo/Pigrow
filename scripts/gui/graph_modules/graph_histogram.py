...
def make_graph(list_of_datasets, graph_path, ymax="", ymin="", size_h="", size_v="", dh="", th="", tc="", dc="", extra=[]):
    ...
    def make_averages(date_list, value_list, key_list):
        # Hourly average
        dictionary_of_sets = {}
    ...
    for x in list_of_datasets:
        ...
        value_list = x[1]
        key_list = x[2]
        hourly_ave_values =  make_averages(date_list, value_list, key_list)
        # Create graph with hourly averages
        n, bins, patches = plt.hist(hourly_ave_values, num_of_bins, density=True, alpha=0.75, label="Hourly Average - " + key_list[0])
        # unhash the next line to make a graph using the raw values instead
        n, bins, patches = plt.hist(value_list, num_of_bins, density=True, alpha=0.75, label="Raw Values" + key_list[0])
