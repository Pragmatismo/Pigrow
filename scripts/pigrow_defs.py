        ...
        if not err_log == False:
            with open(err_log, "a") as f:
                line = 'update_reddit.py @' + \
                    str(datetime.datetime.now()) + '@ settings file error\n'
                f.write(line)
        ...
        with open(loc_settings, "w") as f:
            for a, b in pi_set.iteritems():
                try:
                    ...
                    print(
                        "ERROR SETTINGS FILE ERROR SETTING NOT SAVED _ SERIOUS FAULT!")
    except Exception:
        print("Settings not saved!")
        with open(err_log, "a") as ef:
            line = 'update_reddit.py @' + \
                str(datetime.datetime.now()) + '@ settings file save error\n'
            ef.write(line)
        print("Log writen:" + line)
...
def archive_grow(loc_dic, name, compress=False):
    ...
    responce += "Image Capture Dicrectory; " + \
        str(len(os.listdir(caps_path))) + " files.  \n"
    responce += "Graph Dicrectory; " + \
        str(len(os.listdir(graph_path))) + " files.  \n"
    responce += "Log Dicrectory; " + \
        str(len(os.listdir(log_path))) + " files.  \n"
    d_total, d_used, d_free, d_percent = disk_full(loc_dic['path'])
    responce += "Current Filesystem has " + \
        str(d_free) + " free space, " + str(d_percent) + "% remaining.  \n"
    ...
    if compress == False:
        ...
        for pic in os.listdir(caps_path):
            ...
            if pic in os.listdir(archive_path + "/caps/"):
                ...
                cap_not_copy += 1
        if cap_not_copy > 0:
            responce += "Sorry, copied " + \
                str(cap_copy) + " images but " + \
                str(cap_not_copy) + " pictures didn't copy "
        else:
        ...
        responce += "and " + \
            str(len(os.listdir(archive_path + "/graphs/"))) + " graphs. "
    else:
