...
for argu in sys.argv[1:]:
    if argu == '-h' or argu == '--help' or argu == "-help" or argu == "--h":
        ...
        print("     This version of camcap tests the images to try and make them as similar as ")
        print("     each other as possiible to reduce flicker, especially when outside. ")
        print("")
        print(" set=<filepath>")
...
def attempt_capture(s_val, c_val, g_val, b_val, x_dim, y_dim, cam_num, cam_opt, fsw_extra, uvc_extra, caps_path):
...
if __name__ == '__main__':
    ...
    images = []
    for x in range(0, amount_to_compare):
        filename = attempt_capture(s_val, c_val, g_val, b_val, x_dim, y_dim, cam_num, cam_opt, fsw_extra, uvc_extra, caps_path)
        file = caps_path + filename
        filesize = os.path.getsize(file)
        images.append([filesize, file])
